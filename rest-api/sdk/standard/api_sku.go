...
Create a SKU on the selected Site's NICo Core service.
...
type ApiDeleteSkuRequest struct {
	...
	org              string
	skuId            string
	skuDeleteRequest *SkuDeleteRequest
}

func (r ApiDeleteSkuRequest) SkuDeleteRequest(skuDeleteRequest SkuDeleteRequest) ApiDeleteSkuRequest {
	r.skuDeleteRequest = &skuDeleteRequest
	return r
}
...
Delete an unused SKU from the selected Site's NICo Core service.
...
func (a *SKUAPIService) DeleteSkuExecute(r ApiDeleteSkuRequest) (*http.Response, error) {
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.skuDeleteRequest == nil {
		return nil, reportError("skuDeleteRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.skuDeleteRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}
...
Update selected mutable fields on a SKU owned by the selected Site's NICo Core service.

Fields omitted from the request retain their current Core values. The Org must have an Infrastructure Provider that owns the Site. The user must have an authorization role with the `PROVIDER_ADMIN` suffix.
