...
func (cepsh CreateExpectedPowerShelfHandler) Handle(c echo.Context) error {
	...
	infrastructureProvider, tenant, apiError := common.IsProviderOrTenant(ctx, logger, cepsh.dbSession, org, dbUser, false, nil)
	if apiError != nil {
...
func (gaepsh GetAllExpectedPowerShelfHandler) Handle(c echo.Context) error {
	...
	infrastructureProvider, tenant, apiError := common.IsProviderOrTenant(ctx, logger, gaepsh.dbSession, org, dbUser, true, nil)
	if apiError != nil {
		return cutil.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
...
func (gepsh GetExpectedPowerShelfHandler) Handle(c echo.Context) error {
	...
	// ensure our user is a provider for the org
	infrastructureProvider, tenant, apiError := common.IsProviderOrTenant(ctx, logger, gepsh.dbSession, org, dbUser, true, nil)
	if apiError != nil {
		return cutil.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
...
func (uepsh UpdateExpectedPowerShelfHandler) Handle(c echo.Context) error {
	...
	// Ensure our user is a provider for the org
	infrastructureProvider, tenant, apiError := common.IsProviderOrTenant(ctx, logger, uepsh.dbSession, org, dbUser, false, nil)
	if apiError != nil {
		return cutil.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
...
func (depsh DeleteExpectedPowerShelfHandler) Handle(c echo.Context) error {
	...
	// Ensure our user is a provider for the org
	infrastructureProvider, tenant, apiError := common.IsProviderOrTenant(ctx, logger, depsh.dbSession, org, dbUser, false, nil)
	if apiError != nil {
		return cutil.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
