...
Go-based CLI tools used in the Makefile are version-pinned via blank
imports in `tools/tools.go`. This anchors them in `go.mod`/`go.sum` so
every developer and CI run uses the same binary without a separate
install step.
...
```sh
# From rest-api/
go get github.com/bufbuild/buf/cmd/buf@v1.71.0
go mod tidy

# Update the version comment in tools/tools.go to match
```

Commit `go.mod`, `go.sum`, and `tools/tools.go` together.
...
Tools that can be called directly use `go run` via Makefile variables
