...
impl ExpandQuery {
    ...
    pub fn no_links() -> Self {
        // iDRAC8 rejects the bare `$expand=.` (HTTP 400 IDRAC.1.6.SYS457) and
        // only accepts `$expand=.($levels=1)`. Emit an explicit level so Site
        // Explorer discovery works on iDRAC8; iDRAC9+ accept it too.
        Self {
            expand_expression: ".".to_string(),
            levels: Some(1),
        }
