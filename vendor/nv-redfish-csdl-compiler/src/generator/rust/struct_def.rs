...
impl<'a> StructDef<'a> {
    ...
    fn gen_de_struct_field_serde_annot<T>(
        ...
        if required.into_inner() && nullable.into_inner() {
            ...
            // A required, non-nullable array is `Vec<T>`. Redfish spells an
            // empty array `[]`, but some services send `null` (BlueField-2 BMC
            // 24.10 does so for a collection's `Members`), so accept `null` as
            // an empty array instead of failing the whole resource.
            if matches!(cardinality, OneOrCollection::Collection(_)) {
                quote! { #[serde(rename=#rename, deserialize_with="de_required_collection")] }
            } else {
