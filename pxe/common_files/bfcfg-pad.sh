...
# mlx-mkbfb image header: 24 bytes, first little-endian u64 carries magic 0x13026642.
BFB_HDR_LEN=24
BFB_MAGIC_LE=42660213
...
assert() {
  ...
  # Confirm a plausible BFB header precedes the cfg: at least 24 bytes (the
  # mlx-mkbfb image header) starting with the magic 0x13026642, little-endian
  # on disk. This does NOT validate the BFB: any 24 bytes carrying the magic
  # pass. Record and image validity are mlx-mkbfb's job at build time and the
  # boot ROM's (per-image CRCs) at boot; this check only rules out an artifact
  # that is the tail alone or has arbitrary bytes where a header should be.
  bfb_size=$(wc -c < "$bfb")
  [ "$bfb_size" -ge $((size + BFB_HDR_LEN)) ] \
    || die "assert: $bfb is $bfb_size bytes, fewer than $BFB_HDR_LEN header bytes before the $size-byte cfg"
  bfb_magic=$(head -c 4 "$bfb" | od -An -tx1 | tr -d ' \n')
  [ "$bfb_magic" = "$BFB_MAGIC_LE" ] \
    || die "assert: $bfb does not start with the BFB magic (got $bfb_magic, want $BFB_MAGIC_LE)"
