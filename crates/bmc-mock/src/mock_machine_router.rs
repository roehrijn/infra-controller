...
pub struct MachineRouterOptions {
    pub virtual_media_devices: Vec<VirtualMediaDeviceConfig>,
}
...
pub fn machine_router_with_options(
    machine_info: &MachineInfo,
    callbacks: Arc<dyn Callbacks>,
    mat_host_id: String,
    redfish_auth: bool,
...
fn machine_router_inner(
    ...
    let system_state = Arc::new(
        crate::redfish::computer_system::SystemState::from_config_with_virtual_media(
            system_config,
            options.virtual_media_devices,
        ),
    );
