...
fn generated_mock_config(args: &command_line::Args) -> Result<GeneratedMockConfig, String> {
    ...
    let compatibility_libvirt_mode =
        args.libvirt_domain.is_some() && args.hardware_profile.is_some() && !explicit_mode;
    let legacy_mode = args.libvirt_domain.is_none()
    ...
    if !explicit_mode && !compatibility_libvirt_mode {
        return Err(
    ...
    let machine_role = if compatibility_libvirt_mode {
        MachineRole::Host
    } else {
    ...
    let state_backend = if compatibility_libvirt_mode {
...
fn generated_mock(config: GeneratedMockConfig) -> (Router, BmcState) {
    let libvirt_callbacks = config
        ...
        .map(Arc::new);
    let callbacks: Arc<dyn Callbacks> = if config.legacy_callbacks {
    ...
    {
        bmc_mock::machine_router_with_options(
            &machine_info,
            callbacks,
            String::default(),
            false,
            MachineRouterOptions {
                virtual_media_devices: vec![
...
mod tests {
    ...
    #[test]
    fn preserves_the_pre_libvirt_default_mode() {
        ...
        assert!(config.legacy_callbacks);
    }
