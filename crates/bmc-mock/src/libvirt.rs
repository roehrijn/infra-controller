...
mod tests {
    ...
    use crate::{
        HostHardwareType, MachineRouterOptions, VirtualMediaDeviceConfig,
        machine_router_with_options,
    };
...
"#,
        ...
        let (router, state) = machine_router_with_options(
            &host_info(HostHardwareType::DellPowerEdgeR750),
            callbacks.clone(),
            "test-host-id".to_string(),
            false,
            MachineRouterOptions {
                virtual_media_devices: vec![VirtualMediaDeviceConfig {
                    id: Cow::Borrowed("Cd"),
